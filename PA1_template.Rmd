...
```{r}
    ...
    filter_cond <- split(week_level_data,week_level_data[,c("level","interval")],drop=TRUE)
    ansy_data <- tapply(week_level_data$steps,week_level_data[,c("level","interval")],mean,na.rm=TRUE)
    par(mfrow = c(2,1)) 
    plot.ts(ansy_data["weekday",],ylab="",type="l",col="blue",main="weekday")
    plot.ts(ansy_data["weekend",],ylab="",type="l",col="blue",main="weekend")
    dev.off()
